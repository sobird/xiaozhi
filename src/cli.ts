...
const command = program
  ...
  .version(version)
  .option('-r, --input-sample-rate <Number>', 'Input sample rate of audio', (value: string) => {
    return Number(value) as SampleRate
  })
  .option(
    '-c, --input-channels <Number>',
    'Input number of channels of audio data; e.g. 2 = stereo',
    (value: string) => {
      return Number(value)
    },
  )
  .option('-b, --input-bits <Number>', 'Input encoded sample size in bits', (value: string) => {
    return Number(value)
  })
  .option('-f, --input-frame-duration <Number>', 'Input frame duration', (value: string) => {
    return Number(value)
  })

  .option(
    '-R, --output-sample-rate <Number>',
    'Ouput sample rate of audio',
    (value: string) => {
      return Number(value) as SampleRate
    },
    48000,
  ...
  .option(
    ...
    'Ouput number of channels of audio data; e.g. 2 = stereo',
    (value: string) => {
      return Number(value)
    },
    1,
  ...
  .option(
    '-B, --output-bits <Number>',
    'Ouput encoded sample size in bits',
    (value: string) => {
      return Number(value)
    },
    16,
  ...
  .action(async (options) => {
    ...
    const spinner = ora({
      discardStdin: false,
      spinner: {
        frames: [randomWave(), randomWave(), randomWave(), randomWave()],
